...
class GraficosA(QWidget):
    ...
    # 8 Superfície 3D
    def _build_Superfície_3D(self, form):
        self.eX3=QLineEdit(); form.addRow("X valores:",self.eX3)
        self.eY3=QLineEdit(); form.addRow("Y valores:",self.eY3)
        self.eZ3=QLineEdit(); form.addRow("Z Matrix:",self.eZ3)
    def _plot_Superfície_3D(self):
        X=list(map(float,self.eX3.text().split(',')))
        Y=list(map(float,self.eY3.text().split(',')))
        Z=list(map(float,self.eZ3.text().split(',')))
        nx,ny=len(X),len(Y)
        Zmat=np.array(Z).reshape(ny,nx)
        self.figure = plt.figure(figsize=(6,4)); self.ax = self.figure.add_subplot(111,projection='3d')
        self.canvas.figure = self.figure; self.toolbar.update()
        Xg,Yg=np.meshgrid(X,Y); self.ax.plot_surface(Xg,Yg,Zmat,cmap='viridis')
        self.ax.set_title("Superfície 3D",fontsize=14)
    ...
    # 10 Energia vs Coord.
    def _build_Energia_vs_Coord(self, form):
        self.eXeco=QLineEdit(); form.addRow("Coordenada:",self.eXeco)
        self.eE=QLineEdit(); form.addRow("Energia (kJ/mol):",self.eE)
    def _plot_Energia_vs_Coord(self):
        c=list(map(float,self.eXeco.text().split(',')))
        e=list(map(float,self.eE.text().split(',')))
        if len(c)!=len(e): raise ValueError()
        self.ax.clear(); self.ax.plot(c,e,'-o',color='magenta')
        self.ax.set_title("Energia vs Coord.",fontsize=14)
        self.ax.set_xlabel("Coord."); self.ax.set_ylabel("Energia")
    ...
    # 18 Van’t Hoff
    def _build_Vant_Hoff(self, form):
        self.eInvT2 = QLineEdit(); form.addRow("1/T (1/K):", self.eInvT2)
        self.eLnK2 = QLineEdit(); form.addRow("ln(K):", self.eLnK2)
    def _plot_Vant_Hoff(self):
        invt = list(map(float, self.eInvT2.text().split(',')))
        lnK = list(map(float, self.eLnK2.text().split(',')))
        self.ax.clear(); self.ax.plot(invt, lnK, '-o', color='purple')
        self.ax.set_title("Diagrama de Vant Hoff", fontsize=14)
        self.ax.set_xlabel("1/T (1/K)"); self.ax.set_ylabel("ln(K)")
    ...
    # 22 ΔpH/ΔV Titulação
    def _build_ΔpH_ΔV(self, form):
        self.ePhV = QLineEdit(); form.addRow("∆pH:", self.ePhV)
        self.eVolV = QLineEdit(); form.addRow("∆V:", self.eVolV)
    def _plot_ΔpH_ΔV(self):
        phv = list(map(float, self.ePhV.text().split(',')))
        vol = list(map(float, self.eVolV.text().split(',')))
        self.ax.clear(); self.ax.plot(vol, phv, '-o', color='brown')
        self.ax.set_title("Derivada da Titulação", fontsize=14)
        self.ax.set_xlabel("∆V"); self.ax.set_ylabel("∆pH")
